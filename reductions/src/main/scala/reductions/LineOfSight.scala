...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    (for (i <- from until until) yield {
      if (i == 0) 0 else input(i) / i
    }).max
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    tree match {
      ...
      case Node(lBranch, rBranch) => parallel(
        downsweep(input, output, startingAngle, lBranch),
        downsweep(input, output, max(lBranch.maxPrevious, startingAngle), rBranch)
      )
    }
  }
