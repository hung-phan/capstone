...
object ParallelParenthesesBalancing {
  ...
  def balance(chars: Array[Char]): Boolean = {
    def stackBracket(chars: Array[Char], acc: Int): Int = {
      if (chars.isEmpty || acc < 0) acc
      else if (chars.head == '(') stackBracket(chars.tail, acc + 1)
      else if (chars.head == ')') stackBracket(chars.tail, acc - 1)
      else stackBracket(chars.tail, acc)
    }

    stackBracket(chars, 0) == 0
  }
  ...
  def parBalance(chars: Array[Char], threshold: Int): Boolean = {
    ...
    def reduce(from: Int, until: Int): (Int, Int) = {
      if (until - from <= threshold) traverse(from, until, 0, 0)
        ...
        if (a1 > b2) (a1 + b1 - b2, a2) else (b1, a2 + b2 - a1)
      }
    }
