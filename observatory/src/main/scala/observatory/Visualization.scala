...
object Visualization {
  ...
  def predictTemperature(temperatures: Iterable[(Location, Double)], location: Location): Double = {
    val predictions: Iterable[(Double, Double)] = distanceTemperatureCombine(temperatures, location)
  ...
  def distanceTemperatureCombine(temperatures: Iterable[(Location, Double)], location: Location): Iterable[(Double, Double)] = {
    temperatures.map {
  ...
  def linearInterpolationValue(pointValueMin: Double, pointValueMax: Double, value: Double)(colorValueMin: Int, colorValueMax: Int): Int = {
    round(
      colorValueMin + (colorValueMax - colorValueMin) * (value - pointValueMin) / (pointValueMax - pointValueMin)
    ).toInt
  }
    ...
    val pixels = (0 until imageHeight * imageWidth).par.map {
      pos =>
        pos -> interpolateColor(
          colors,
          predictTemperature(temperatures, locationMap(pos))
        ).pixel()
    }
      .seq
      .sortBy(_._1)
